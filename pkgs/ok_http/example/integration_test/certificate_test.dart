...
void main() async {
  ...
  group('TLS', () {
    group('loadPrivateKeyAndCertificateChainFromPKCS12', () {
      test('success', () async {
        final certBytes = await loadCertificateBytes(
          'test_certs/test-combined.p12',
        );
        final (key, chain) = loadPrivateKeyAndCertificateChainFromPKCS12(
          certBytes,
          '1234',
        );
        expect(
    ...
    test('unknown server cert', () async {
      ...
      final config = const OkHttpClientConfiguration(
      ...
      expect(
        ...
        throwsA(
          isA<io.HandshakeException>().having(
            (e) => e.message,
            'message',
            contains('Handshake'),
          ),
    ...
    test('client cert', () async {
      ...
      expect(
        (await clientCertificate.future)!.issuer,
