...
class HttpIncoming extends Stream<Uint8List> {
  ...
  // Common properties.
  final CustomizedHttpHeaders headers;
  bool upgraded = false;
...
/// the current HttpIncoming or the _parser itself.
///
...
// Whether a HTTP request is being parsed (as opposed to a response).
  ...
  CustomizedHttpHeaders? _headers;
...
// The current incoming connection.
  ...
  factory HttpParser.requestParser() => HttpParser._(true);

  factory HttpParser.responseParser() => HttpParser._(false);
...
// Check whether this response will never have a body.
        ...
        case _State.HEADER_START:
          _headers = CustomizedHttpHeaders(version!);
          if (byte == CharCode.CR) {
...
// Checks whether (_remainingContent * 16 + value) overflows.
        ...
        case _State.BODY:
          ...
          _index = _index - 1;
          var buffer = _buffer!;
          var dataAvailable = buffer.length - _index;
...
// If the connection is idle the HTTP stream is closed.
    if (_state == _State.START) {
      if (!_requestParser) {
        _reportHttpError(
            const HttpException('Connection closed before full header was'
                ' received'));
      }
      _controller.close();
      return;
...
// throw the error.
      _reportHttpError(
          const HttpException('Connection closed before full '
              'header was received'));
      _controller.close();
      return;
    }
...
// throw the error.
  ...
  T valueOfNonNullableParamWithDefault<T extends Object>
...
// Simulate detached by marking as upgraded.
  ...
  static bool isTokenChar(int byte) => byte > 31 && byte < 128 &&
      !_Const.SEPARATOR_MAP[byte];

  static bool isValueChar(int byte) => (byte > 31 && byte < 128) ||
      (byte == CharCode.HT);
  ...
  static int _toLowerCaseByte(int x) => (((x - 0x41)
        & 0x7f) < 26) ? (x | 0x20) : x;
...
// expected should already be lowercase.
  ...
  void _expect(int val1, int val2) {
    if (val1 != val2) {
      throw HttpException('Failed to parse HTTP, $val1 does '
          'not match $val2');
    }
  }
  ...
  void _reportSizeLimitError() {
    ...
    throw HttpException('$method exceeds the '
        '$_headerTotalSizeLimit size limit');
  }
